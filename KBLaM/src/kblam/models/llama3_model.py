...
The KBLaM LLaMA-based model. This docstring replaces the original LLAMA_START_DOCSTRING
removed in transformers>=4.48. Used only for documentation decorators.
"""
_CONFIG_FOR_DOC = "LlamaConfig"
LLAMA_INPUTS_DOCSTRING = """
Input format for the LLaMA-based KBLaM model.
Args:
    input_ids (torch.LongTensor of shape (batch_size, sequence_length)): Token ids.
    attention_mask (torch.FloatTensor of shape (batch_size, sequence_length), optional): Attention mask.
    
"""

class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """
    Backwards-compatible approximation of the old HF LlamaLinearScalingRotaryEmbedding.

    In older transformers versions this class linearly scaled positions by 1/scaling_factor.
    Newer versions fold this into LlamaRotaryEmbedding via rope scaling; this wrapper
    reproduces the older behavior well enough for KBLaM's rope extension logic.
    """
    ...
    def forward(self, x: torch.Tensor, seq_len: int | None = None):
        if seq_len is None:
            seq_len = x.shape[-2]
        # effective linear scaling of positions
        scaled_seq_len = int(math.ceil(seq_len / self.scaling_factor))
        return super().forward(x, scaled_seq_len)

class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """
    Backwards-compatible approximation of the old HF LlamaDynamicNTKScalingRotaryEmbedding.

    The original implementation changed the RoPE base as a function of scaling_factor.
    We mimic that behavior closely enough for KBLaM usage.
    """
    ...
    def __init__(
        ...
        self.scaling_factor = float(scaling_factor)
        # Approximation of the original NTK base rescaling
        # base' = base * scaling_factor ** (dim / (dim - 2))
        if dim > 2 and scaling_factor != 1.0:
            base = base * (self.scaling_factor ** (dim / (dim - 2)))
        super().__init__(
            dim=dim,
            max_position_embeddings=max_position_embeddings,
            base=base,
