...
from kblam.utils.data_utils import augment_row as aug_row
...
def perform_eval(
    ...
    if test_dataset is None:
        kb_idx = np.random.choice(len(kb_retriever.dataset), kb_size)
        test_kb = [kb_retriever.dataset[idx] for idx in kb_idx]
        kb_embedding = kb_retriever.get_key_embeddings(kb_idx)
    else:
        kb_idx = np.random.choice(len(test_dataset), kb_size)
        test_kb = [test_dataset[idx] for idx in kb_idx]
        kb_embedding = kb_retriever.get_key_embeddings(np.arange(len(kb_retriever.dataset)))
    if eval_mode == "icl":
...
gen_parser.add_argument(
    "--train_dataset", type=str, help="Source of training KB (assumes KV pair format)"
)
...
def eval_generate():
    ...
    test_dataset = json.load(open(os.path.join(dataset_dir, test_dataset)))
    if train_dataset is None:
        train_dataset = test_dataset
        test_dataset = None
    else:
        train_dataset = json.load(open(os.path.join(dataset_dir, train_dataset)))
